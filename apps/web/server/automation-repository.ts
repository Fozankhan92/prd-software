...
export type AutomationExecution = { id: string; tenantId: string; ruleId: string; eventType: string; recordId: string; matched: boolean; result: AutomationResult; createdAt: string };
...
export function createSqlAutomationRepository(db: { query<T = unknown>(text: string, values?: readonly unknown[]): Promise<{ rows: T[] }> }): AutomationRepository {
  return {
    ...
    async recordExecution(execution) {
      await db.query(`insert into crm_automation_execution (id, tenant_id, rule_id, event_type, record_id, matched, result) values ($1, $2, $3, $4, $5, $6, $7::jsonb)`, [execution.id, execution.tenantId, execution.ruleId, execution.eventType, execution.recordId, execution.matched, JSON.stringify(execution.result)]);
    },
...
export function executionFromEvent(event: AutomationEvent, result: AutomationResult, id = crypto.randomUUID()): AutomationExecution {
  return { id, tenantId: event.tenantId, ruleId: result.ruleId, eventType: event.type, recordId: event.recordId, matched: result.matched, result, createdAt: new Date().toISOString() };
}
