...
function expandedRecordValue(record: WebCrmRecord, label: string): string {
  return expandedRecordRows(record).find((row) => row.label.toLowerCase() === label.toLowerCase())?.value ?? '—';
}
...
export function WebApp() {
  ...
  const visibleCrmViewRecords = useMemo(() => {
    if (expandedRecordFilter === 'open') return crmViewRecords.filter((record) => !['Completed', 'Closed', 'Cancelled', 'Lost', 'Inactive'].includes(record.status));
    if (expandedRecordFilter === 'attention') return crmViewRecords.filter((record) => ['Urgent', 'High', 'Overdue', 'Escalated', 'At risk'].some((term) => record.status.includes(term)) || expandedRecordValue(record, 'Date') !== '—');
    if (expandedRecordFilter === 'followup') return crmViewRecords.filter((record) => expandedRecordValue(record, 'Date') !== '—');
    if (expandedRecordFilter === 'unassigned') return crmViewRecords.filter((record) => expandedRecordValue(record, 'Owner') === '—');
    if (expandedRecordFilter === 'detailed') return crmViewRecords.filter((record) => expandedRecordValue(record, 'Details') !== '—');
    if (expandedRecordFilter === 'recent') return [...crmViewRecords].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
    return crmViewRecords;
  }, [crmViewRecords, expandedRecordFilter]);
