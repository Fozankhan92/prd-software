...
const recordFormConfigs: Record<WebCrmRecord['kind'], RecordFormConfig> = {
  ...
  'service-case': { nameLabel: 'Case subject', namePlaceholder: 'Customer issue', fields: [{ key: 'priority', label: 'Priority', placeholder: 'Low, normal, high, urgent' }, { key: 'sla', label: 'SLA target', placeholder: 'Response/resolution target' }, { key: 'queue', label: 'Service queue', placeholder: 'Queue or owner' }], statuses: ['New', 'Open', 'Pending', 'Resolved', 'Closed'] },
  complaint: { nameLabel: 'Complaint subject', namePlaceholder: 'Complaint subject', fields: [{ key: 'category', label: 'Complaint category', placeholder: 'Product, service, billing...' }, { key: 'resolution', label: 'Resolution plan', placeholder: 'Resolution details' }], statuses: ['New', 'Investigating', 'Resolved', 'Closed'] },
  campaign: { nameLabel: 'Campaign name', namePlaceholder: 'Campaign name', fields: [{ key: 'channel', label: 'Channel', placeholder: 'Email, event, advertising...' }, { key: 'budget', label: 'Budget', placeholder: 'Budget amount', type: 'number' }, { key: 'startDate', label: 'Start date', placeholder: '', type: 'date' }], statuses: ['Draft', 'Planned', 'Active', 'Completed', 'Cancelled'] },
...
export function WebApp() {
  ...
  function addRecord(event: React.FormEvent<HTMLFormElement>) {
    ...
    const detailParts = [recordDetail.trim(), ...dynamicDetails, recordRelationship.trim() ? `Related to: ${recordRelationship.trim()}` : '', recordOwner.trim() ? `Owner: ${recordOwner.trim()}` : '', recordAmount.trim() ? `Amount: ${recordAmount.trim()}` : '', recordDate ? `Date: ${recordDate}` : '', crmView === 'service' ? `Priority: ${recordPriority}` : ''].filter(Boolean);
  ...
  function changeCrmView(view: (typeof crmViews)[number][0]) {
    setCrmView(view);
    changeRecordKind(crmViewKinds[view][0]);
  }
      ...
      <main className="web-app__main">
        ...
        <section className="web-app__content">
          ...
          {activeModule === 'home' ? <>
            ...
            {crmView === 'overview' ? <section className="web-card web-customer-360"><div className="web-card__heading"><div><p className="web-app__eyebrow">Customer 360</p><h3>Complete customer view</h3></div><span className="web-card__hint">{workspace.records.length} linked records</span></div><label className="web-form__customer-select">Select customer or organization<select value={selectedCustomerId} onChange={(event) => setSelectedCustomerId(event.target.value)}><option value="">Choose a customer</option>{workspace.records.filter((record) => ['customer', 'organization', 'contact'].includes(record.kind)).map((record) => <option value={record.id} key={record.id}>{record.name} · {recordLabels[record.kind]}</option>)}</select></label>{selectedCustomerId ? <div className="web-customer-360__timeline">{workspace.records.filter((record) => record.id === selectedCustomerId || record.detail.includes(workspace.records.find((item) => item.id === selectedCustomerId)?.name ?? '')).map((record) => <article key={record.id}><span className="web-record-icon">{recordLabels[record.kind].slice(0, 1)}</span><div><strong>{record.name}</strong><small>{recordLabels[record.kind]} · {record.status}</small><p>{record.detail || 'No additional details.'}</p></div></article>)}</div> : <p className="web-empty">Select a customer, organization, or contact to see related sales, activities, service, contracts, and statements.</p>}</section> : null}
          </> : null}
