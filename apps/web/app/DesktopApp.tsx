...
export function DesktopApp({ permissions = {} }: DesktopAppProps) {
  ...
  return (
    <DesktopShell navItems={visibleNavigation} activeItem={active?.id ?? "home"} onNavigate={(item: DesktopNavpprovals"], metrics: ["Employees", "Departments", "Leave requests", "Pending approvals"] },
...
export function DesktopApp({ permissions = {} }: DesktopAppProps) {
   ...
   return (
      <DesktopShell navItems={visibleNavigation} activeItem={active?.id ?? "home"} onNavigate={(item: DesktopNavItem) => setActiveItem(item.id)}>
         <section aria-labelledby="workspace-heading">
          <p className="desktop-shell__eyebrow">{content.eyebrow}</p>p>
          <h2 id="workspace-heading">{content.title}</h2>h2>
          <p>{content.summary}</p>p>
           {content.metrics ? <div className="desktop-workbench__metrics" aria-label={content.title + " summary"}>{content.metrics.map((metric) => <article key={metric}><strong>—</strong>strong><span>{metric}</span>span></article>article>)}</div>div> : null}
          <div className="desktop-workbench__actions" aria-label={content.title + " actions"}>
            {content.actions.map((action) => <button type="button" key={action}>{action}</button>button>)}
          </div>div>
         </section>section>
      </DesktopShell>DesktopShell>
      );
}
</section>Item) => setActiveItem(item.id)}>
      <section aria-labelledby="workspace-heading">
